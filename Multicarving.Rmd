...
By exchanging $y=X\beta_0+\sigma^2\epsilon$ for $y=X\beta_0+\sigma\epsilon$ in the simulation setup we finally managed to get uniformly distributed p-values, which can be observed in \@ref(fig:Histo_pvals)

```{r Histo_pvals, echo=FALSE,fig.cap="Empirical pdf of coefficients of $\\beta^{Split}$ for s=1143", out.width = "450px"}
knitr::include_graphics("plots/histogram_and_cdf_of_screened_pvals_N=3000.jpeg")
```


Now that the distribution of the p-values seemed to be correct.
