...
* End of $\hat \beta^{PoSI}$: Here we wrote the distribution and the conditions for the truncated normal very formally. Is it ok to write a sentence after that, saying: From here onwards, we only write $\hat\beta \sim TN(..., \mathcal{V^-}(y))$ (i.e. only the conditioning, Drysdale also doesn't do it for example).
...
* Explain how and that the $A_0$ part in $\hat \beta^{PoSI}$ becomes 0.

* Exchange all the cardinalities $\hat s$ by $\hat m$ (both for the general case in the PoSI section and also in the cases of $\hat M_A$)

* Check whether all of the $\hat{M}$ are really consistent 

* Define $\hat \beta^{PoSI}$ properly

* Add in each section which error we control (see his comment next to 2.5.0.1)
...
* Change FWER dots to symbols and power lines to something visible in black and white format
