...
for (fraq_ind in 1:f){
  ...
  # test_res_D <- rep(0,4)
  # test_res_C <- rep(0,4)
  test_res_split <- rep(0,4)
  counter <- 0
  # powers_D <- rep(0,nsim)
  # powers_C <- rep(0,nsim)
  powers_split <- rep(0,nsim)
  # type1_error_D <- rep(0,nsim)
  # type1_error_C <- rep(0,nsim)
  type1_error_split <- rep(0,nsim)
  ...
  for (i in 1:nsim){
    ...
    H0T_Rej_split<-sum(p_vals_split_fwer<=0.05 & beta==0)
    H0F_Rej_split<-sum(p_vals_split_fwer<=0.05 & beta==1)
    H0T_N_Rej_split<-sum(p_vals_split_fwer>0.05 & beta==0)
    H0F_N_Rej_split<-sum(p_vals_split_fwer>0.05 & beta==1)
    ...
    test_res_split<- test_res_split + c(H0T_Rej_split,H0F_Rej_split,H0T_N_Rej_split,H0F_N_Rej_split)
    ...
    powers_split[i] <- H0F_Rej_split/(length(sel.index))
    
    # type1_error_D[i] <- H0T_Rej_D/(p-length(sel.index))
    # type1_error_C[i] <- H0T_Rej_C/(p-length(sel.index))
    type1_error_split[i] <- H0T_Rej_split/(p-length(sel.index))
...
# --------------- Create plots --------------
load("myEnvironment_nsim200_6fraqs.RData")
