...
saturated_path<-paste(Local_path, "/carve_saturated.R", sep="")
...
frac.vec <- c(0.75) # selection fraction
nsim <- 2
ntasks <- nsim
...
for (frac in frac.vec) {
  ...
  res<-foreach(gu = 1:nsim, .combine = rbind,
               .packages = c("MASS", "selectiveInference", "glmnet", "Matrix",
                             "hdi",
                             #"tmg",
                             "truncnorm"), .options.snow=opts) %dorng%{
                               ...
                               mcrtry <- tryCatch_W_E(carve.saturated(x, y, B = B, fraction = frac, ci.level = 0.95, 
