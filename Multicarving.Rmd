...
We implemented a new version of the parallelized power study, which is supposed to test $\hat \beta^{Drysdale}$ and also $\hat \beta^{POSI}$ and $\hat \beta^{Split}$ in a different manner to before. The idea is the following:

* All of $\hat \beta^{Drysdale}, \hat \beta^{Carve}, \hat \beta^{POSI}$ and $\hat \beta^{Split}$ start on the same fraction. Let $s:=\#M_A$ and let $f \in [0,1]$ be the the fraction of the observations that are being used for variable selection. Therefore $\hat \beta^{Drysdale}$ can not be computed due to rank deficiency, if $s>\min\{n*f, n*(1-f)\}$. If that happens in a specific run of the simulation, we set f=f-0.025 and run the Lasso again for variable selection. We repeat this process until $\hat \beta^{Drysdale}$ can be computed.

* The idea of this setup is to compare $\hat \beta^{Drysdale}$ to $\hat \beta^{Carve}$ while having both in their respectively optimal environments as far as the selection-inference split is concerned. Since $\hat \beta^{Drysdale}$ can not be computed for splits where $f$ is close to $1$, because to little observations are left to deal with the amount of active variables, we expect to see that as $f$ rises, the $\hat \beta^{Drysdale}$ will have to reduce its fraction more and more and therefore be less likely to fulfill the screening condition. Thus, setting p-values to 1 for some truly active variables, would in theory result in lower power for very high fractions for $\hat \beta^{Drysdale}$ than in the previous simulation.

## Paul 3rd of May 2024

* Corrected the condition under which $\hat \beta^{Drysdale}$ can not be calculated. Due to the Lasso only choosing $s\leq \min\{n_A,p\}$ variables, we only need the condition $s \leq n*(1-f)$ instead of $s \leq \min\{n*f, n*(1-f)\}$, since $s \leq n*(1-f)$ is already guaranteed anyways.
...
We conclude, that for our purposes it is therefore sufficient to replace NaNs with 1s, whenever they occur.
