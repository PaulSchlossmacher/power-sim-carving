...
carve.linear <- function(x, y, split, beta, lambda, sigma,
                         normalize_truncation_limits = FALSE){
  #Split the data
  n <- length(y)
  p <- length(beta)
  n.a <- length(split)
...
beta.posi <- function(x, y, split, beta, lambda,
  ...
  return(list(pvals=pvals, norm_consts=norm_consts, tau.2 = tau.2, vlo = vlo,
              vup = vup, beta_posi=beta_posi))
}