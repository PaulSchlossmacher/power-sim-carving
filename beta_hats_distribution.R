...
act.index <- round(seq(from=1, to=p, length.out=350),0)#active predictors
beta <- rep(0, p)
beta[act.index] <- 1
set.seed(41)
...
plot(density(beta_split), main="Empirical PDF beta_split whole, s=1143", xlab="Values", ylab="Density")
plot(density(beta_posi), main="Empirical PDF beta_posi whole, s=1143", xlab="Values", ylab="Density")

length(beta_split)

beta_split_not_active_selected<-beta_split[act.index]
plot(density(beta_split_active), main="Empirical PDF Test", xlab="Values", ylab="Density")


sel.index <- which(beta_tmp != 0)


beta_split[sel.index][!(sel.index %in% act.index)]
